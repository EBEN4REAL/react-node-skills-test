...
-- UPDATED: Change roll from INTEGER to VARCHAR(50)
CREATE TABLE IF NOT EXISTS user_profiles (
    ...
    section_name VARCHAR(50) REFERENCES sections(name)
        ...
        ON DELETE SET NULL
        DEFAULT NULL,
    roll VARCHAR(50) DEFAULT NULL,   -- Changed here from INTEGER to VARCHAR(50)
...
ALTER COLUMN roll TYPE VARCHAR(50) USING roll::text;
...
BEGIN
    ...
    -- staff role cannot be 3 (student)
    IF _role = 3 THEN
        RETURN QUERY
...
DECLARE
    ...
    _sectionName TEXT;
    _admissionDt DATE;
    _roll TEXT;  -- Updated to TEXT so it accepts string values
BEGIN
    _roleId := 3; -- hard-coded role for student
    _userId := COALESCE((data ->>'userId')::INTEGER, NULL);
    _name := COALESCE(data->>'name', NULL);
    _gender := COALESCE(data->>'gender', NULL);
    _phone := COALESCE(data->>'phone', NULL);
    ...
    _roll := COALESCE(data->>'roll', NULL);  -- No integer cast
    ...
    SELECT teacher_id
    FROM class_teachers
    WHERE class_name = _className 
      AND section_name = _sectionName
    INTO _reporterId;
...
BEGIN
    ...
    IF _user_role_id = 1 THEN
        ...
        END IF;
    ELSE
        _student_count_current_year := 0;
        _student_perc_comparison := 0;
        _student_value_comparison := 0;

        _teacher_count_current_year := 0;
        _teacher_perc_comparison := 0;
        _teacher_value_comparison := 0;

        _parent_count_current_year := 0;
        _parent_perc_comparison := 0;
        _parent_value_comparison := 0;
    END IF;
    ...
    WITH _celebrations AS (
        ...
        SELECT 
            ...
            'Happy ' ||
              CASE
                WHEN t1.role_id = 3 THEN EXTRACT(YEAR FROM age(now(), t2.admission_dt))
                ELSE EXTRACT(YEAR FROM age(now(), t2.join_dt))
              END || ' Anniversary!' AS event,
            CASE
              WHEN t1.role_id = 3 THEN t2.admission_dt
              ELSE t2.join_dt
            END AS "eventDate"
        FROM users t1
        JOIN user_profiles t2 ON t1.id = t2.user_id
    ...
    FROM (
        ...
        JOIN _month_dates t4 ON t2.from_dt <= t4.day_end AND t2.to_dt >= t4.day_start
        WHERE t2.status = 2
    ) t;
